...
const { encryptedFile, encryptedFileName, loading, error, encryptFile } = useRc5()
...
async function onSubmit() {
  ...
  await encryptFile(inputFile.value, inputPassword.value)
}

async function onDownload() {
  await saveUrlAs(encryptedFile.value, encryptedFileName.value ?? 'encrypted.rc5')
}
</script>

<template>
  <main class="px-20 py-10">
    <h1 class="text-4xl font-bold">Лабораторна робота 3: RC5 Шифрування Файлу</h1>
